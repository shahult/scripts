    ...
    foreach ($Row in $AccountList) {
        ...
        try {
            ...
            if ($ConfigureObjectReplication -and ($SourceKind -notin @("StorageV2", "BlockBlobStorage") -or $SourceHns -eq $true)) {
                ...
                if ($ExistingPoliciesJson) {
                    ...
                    foreach ($Pol in $ExistingPolicies) {
                        ...
                        if ($PolSource -eq $SourceProps.id -or $PolSource -eq $Source.AccountName) {
                            ...
                            # Store existing ruleIds by container name for preservation
                            if ($Pol.properties.rules) {
                                foreach ($Rule in $Pol.properties.rules) {
                                    $ContainerKey = $Rule.sourceContainer
                                    $ExistingRules[$ContainerKey] = $Rule.ruleId
                                }
                                Write-Log "    Existing policy has $($Pol.properties.rules.Count) rule(s). Will preserve ruleIds." "" $Progress
                            }
                            break
                        }
                    }
                ...
                # If updating existing policy, preserve existing ruleIds; otherwise generate new ones
                ...
                foreach ($Container in $UserContainers) {
                    ...
                    # Use existing ruleId if available, otherwise generate new one
                    $RuleId = if ($ExistingRules.ContainsKey($ContainerName)) {
                        $ExistingRules[$ContainerName]
                    } else {
                        "rule$RuleCounter"
                    }
                ...
                Write-Log "    Creating policy with $($Rules.Count) rule(s)..." "" $Progress

                $TempFile = [System.IO.Path]::GetTempFileName()
                $PolicyPayload | Out-File -FilePath $TempFile -Encoding UTF8

                # Create or update the policy on the destination account
                $PolicyResult = az rest --method PUT --url $DestPolicyEndpoint --body "@$TempFile" -o json 2>&1

                if ($LASTEXITCODE -ne 0 -or $PolicyResult -like "*error*") {
                    Write-Log "    ERROR: Failed to create policy on destination." "ERROR" $Progress
                    Write-Log "    Error details: $PolicyResult" "ERROR" $Progress
                    # Uncomment below to see full payload for debugging:
                    # Write-Log "    Payload sent: $PolicyPayload" "ERROR" $Progress
                ...
                if ($PolicyResult) {
                    ...
                    # First, check if source already has this policy (from previous run)
                    az account set --subscription $Source.SubscriptionId | Out-Null
                    Write-Log "    Checking for existing policy on source account..." "" $Progress
                    
                    $SourceExistingPoliciesJson = az rest --method GET --url "https://management.azure.com$($SourceProps.id)/objectReplicationPolicies?api-version=2024-01-01" -o json 2>$null
                    $SourceHasPolicy = $false
                    $SourceExistingRules = @{}
                    
                    if ($SourceExistingPoliciesJson) {
                        $SourceExistingPolicies = ($SourceExistingPoliciesJson | ConvertFrom-Json -AsHashtable).value
                        foreach ($SourcePol in $SourceExistingPolicies) {
                            if ($SourcePol.properties.policyId -eq $PolicyId) {
                                $SourceHasPolicy = $true
                                Write-Log "    Source already has this policy (ID: $PolicyId). Will update." "" $Progress
                                
                                # Get existing ruleIds from source for this policy
                                if ($SourcePol.properties.rules) {
                                    foreach ($Rule in $SourcePol.properties.rules) {
                                        $ContainerKey = $Rule.sourceContainer
                                        $SourceExistingRules[$ContainerKey] = $Rule.ruleId
                                    }
                                }
                                break
                            }
                    ...
                    # Rebuild rules using source's existing ruleIds if it has the policy
                    $SourceRules = @()
                    $SourceRuleCounter = 1
                    foreach ($Container in $UserContainers) {
                        $ContainerName = $Container.ToString().Trim()
                        
                        # Priority: Use source's existing ruleId, then destination's, then generate new
                        $SourceRuleId = if ($SourceExistingRules.ContainsKey($ContainerName)) {
                            $SourceExistingRules[$ContainerName]
                        } elseif ($ExistingRules.ContainsKey($ContainerName)) {
                            $ExistingRules[$ContainerName]
                        } else {
                            "rule$SourceRuleCounter"
                        }
                    ...
                    # Build source policy payload with proper rules
                    # IMPORTANT: Include the policyId in the payload for the source
                    $SourcePolicyPayload = @{
                        properties = @{
                            policyId           = $PolicyId
                            sourceAccount      = $SourceProps.id
                    ...
                    # Apply to source
                    # IMPORTANT: When source doesn't have this policy yet, ALWAYS use /default endpoint
                    # The policyId in the payload will tell Azure to match it with the destination's policy
                    $SourcePolicyEndpoint = if ($SourceHasPolicy) {
                        "https://management.azure.com$($SourceProps.id)/objectReplicationPolicies/$PolicyId`?api-version=2024-01-01"
                        Write-Log "    Source has existing policy - updating existing policy $PolicyId" "" $Progress
                    } else {
                        "https://management.azure.com$($SourceProps.id)/objectReplicationPolicies/default?api-version=2024-01-01"
                        Write-Log "    Source has NO existing policy - creating new policy with ID $PolicyId" "" $Progress
                    }
                    
                    Write-Log "    Applying policy to source account (subscription: $($Source.SubscriptionId))..." "" $Progress
                    Write-Log "    Source endpoint: $SourcePolicyEndpoint" "" $Progress
                    Write-Log "    Number of rules: $($SourceRules.Count)" "" $Progress
                    
                    # DEBUG: Show payload (uncomment to troubleshoot)
                    # Write-Log "    DEBUG - Payload being sent to source:" "WARN" $Progress
                    # Write-Log $SourcePolicyPayload "WARN" $Progress
                    
                    $SourcePolicyResult = az rest --method PUT --url "$SourcePolicyEndpoint" --body "@$TempFile" -o json 2>&1
                    
                    if ($LASTEXITCODE -eq 0 -and $SourcePolicyResult -notlike "*error*" -and $SourcePolicyResult -notlike "*Invalid*") {
                        ...
                        Write-Log "    ✗ FAILED to apply Object Replication policy to SOURCE account." "ERROR" $Progress
                        Write-Log "    Exit code: $LASTEXITCODE" "ERROR" $Progress
                        Write-Log "    Error details:" "ERROR" $Progress
                        Write-Log "    $SourcePolicyResult" "ERROR" $Progress
                        Write-Log "" "ERROR" $Progress
                        Write-Log "    TROUBLESHOOTING:" "WARN" $Progress
                        Write-Log "    1. Check permissions: Need 'Storage Account Contributor' on SOURCE account" "WARN" $Progress
                        Write-Log "    2. Verify source account type: Must be StorageV2 or BlockBlobStorage (not BlobStorage/FileStorage)" "WARN" $Progress
                        Write-Log "    3. Check HNS: Source must NOT have Hierarchical Namespace enabled" "WARN" $Progress
                        Write-Log "    4. Subscription access: Verify you're authenticated to source subscription" "WARN" $Progress
                        Write-Log "" "WARN" $Progress
                        Write-Log "    NOTE: Destination has policy but source does not - replication will NOT work!" "WARN" $Progress
